...
public class GraphLayout {
	...
	public void generateGraph() {
		...
		for (Junction j : rm.getJunctions()) {
			...
			//Convertir Junction a Node en aristas
			js.put(j, n); 
			g.addNode(n);
		}
		for (Road r : rm.getRoads()) {
			...
			green = r == r.getEnd().getJunctionDeque().get(r.getEnd().getTrafficLight()).
					getRoad() ? true : false;
			Edge e = new Edge(r.getId(), js.get(r.getStart()), js.get(r.getEnd()),
					r.getSize(), green);
