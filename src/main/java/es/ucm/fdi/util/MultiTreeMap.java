...
public class MultiTreeMap<K, V> extends TreeMap<K, ArrayList<V>> {
	...
	/**
	 * Removes the first occurrence of a value from the list found at a given key.
	 * Efficiency is O(size-of-that-list)
	 * 
	 * @param key
	 *            to look into
	 * @param value
	...
	/**
	 * Iterates through all internal values (not the arraylists themselves), first
	 * by key order, and within each bucket, by insertion order.
	 */
